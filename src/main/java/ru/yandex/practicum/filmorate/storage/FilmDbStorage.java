...
public class FilmDbStorage implements FilmStorage {
    ...
    public Film createFilm(Film film) {
        ...
        if (film.getGenres() != null) {
            ...
            film.getGenres().forEach((Genre genre) ->
    ...
    public Film updateFilm(Film film) {
        String sqlQuery =
                "UPDATE films SET " +
                        "description = ?, rating_id = ?,duration = ?, name = ?, release_date = ? " +
                        "where id = ?";
    ...
    public Film getFilmById(int filmId) {
        String sqlQuery = "SELECT f.id, f.name, f.description, f.release_date, f.duration," +
                " f.rating_id, r.rating_name, fg.genre_id, g.genre FROM films AS f LEFT JOIN rating AS r ON f.rating_id = r.rating_id LEFT JOIN film_genre AS fg ON f.id=fg.film_id LEFT JOIN genre AS g ON fg.genre_id=g.genre_id WHERE id = " + filmId + " ;";
        try {
    ...
    public List<Film> getPopularFilms(int count) {
        String sql = "SELECT f.id, f.name, f.description, f.release_date, f.duration, " +
                ...
                "LEFT JOIN rating AS r ON f.rating_id = r.rating_id LEFT JOIN film_genre AS fg ON f.id=fg.film_id LEFT JOIN genre AS g ON fg.genre_id=g.genre_id " +
                "GROUP BY f.id " +
                "ORDER BY count DESC " +
                "LIMIT " + count + ";";
        List<Film> films = jdbcTemplate.query(sql, filmsExtractor);
